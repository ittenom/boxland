...
- [ ] **Step 4: Verify Burrito binaries still build**

```bash
MIX_ENV=prod mix release 2>&1 | tail -5
```

Expected: both targets build successfully.

- [ ] **Step 5: Smoke-test the macOS binary**

```bash
# Open the TUI briefly:
timeout 3 ./burrito_out/boxland_macos_arm64 || true
echo "exit code: $?"
```

Expected: TUI opens, shows menu (we won't see it in the captured output because timeout kills it), exits with non-zero (that's normal — timeout did the killing). The fact that it didn't crash on launch is the success signal.
...
- `MIX_ENV=prod mix release` builds both Mac arm64 + Linux amd64 single-file binaries to `burrito_out/`
- The macOS binary opens the TUI menu, exits cleanly on Quit
- 9-stage Install workflow is fully tested (mock + real e2e via Docker)
- All foundation tests still pass
- README explains how to download + run the binaries
