...
- [ ] **Step 2: Update Dockerfile CMD to default to TUI**

Edit `Dockerfile`. Find the `CMD ["bin/boxland", "start"]` line at the end. Change to:
...
(Drop the `"start"` argument so the argv dispatch in `Boxland.Application` routes to `Boxland.TUI.Server.start_link/0` instead of trying `start` as a subcommand.)
...
- [ ] **Step 6: Rebuild the image with the new CMD + templates**
...
- [ ] **Step 7: Verify the launcher runs the TUI inside a container**

Run a quick non-blocking sanity check:

```bash
./bin/boxland --version 2>&1 | head -5
```

Expected: prints `boxland 0.1.0`. If you get `Unknown command:` errors, the argv dispatch isn't matching `--version` correctly — verify `lib/boxland/application.ex` has the `["--version"]` clause from Plan Task 19.

- [ ] **Step 8: Commit**

```bash
git add Dockerfile priv/templates/ bin/boxland
