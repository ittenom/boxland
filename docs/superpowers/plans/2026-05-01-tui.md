...
defmodule Boxland.Server.SupervisorTest do
  ...
  setup do
    # Application.start auto-starts ServerSup; ensure it's running.
    pid = Process.whereis(ServerSup)
    assert is_pid(pid)
    {:ok, pid: pid}
  end

  test "starts with zero children", %{pid: pid} do
...
{:ok, _} = Boxland.Server.Supervisor.start_children()
```
...
## Phase 7 — Burrito packaging

### Task 20: Add Burrito dep + releases config + verify build

**Files:**
- Modify: `mix.exs` (add dep + releases config)
- Modify: `.gitignore`

- [ ] **Step 1: Add Burrito dep**
...
- [ ] **Step 2: Add releases config**

In `mix.exs`, locate the `releases` keyword in `def project do ... end`. If absent, add it. The full project block should include:
...
- [ ] **Step 3: Add `burrito_out/` to `.gitignore`**

Edit `.gitignore`. Append:

```
# Burrito-built single-file binaries
burrito_out/
```

- [ ] **Step 4: Install Burrito's Zig toolchain**

```bash
mix burrito.install
```

Expected: downloads Zig (~70MB) and target ERTS tarballs (~50MB each). Cached in `~/.local/share/.burrito_lib/`. First-time setup; subsequent builds reuse. May take 2-5 min.
...
Expected: builds both targets; ~3-10 min the first time. Output:
```
==> Wrapping release...
==> Wrapped: burrito_out/boxland_macos_arm64
==> Wrapped: burrito_out/boxland_linux_amd64
```

If build fails, STOP and report BLOCKED with the last 30 lines.

- [ ] **Step 6: Verify the macOS binary runs**

```bash
./burrito_out/boxland_macos_arm64 --version
```

Expected: prints `boxland 0.1.0`. The first launch will take 1-2s as Burrito self-extracts.

- [ ] **Step 7: Commit**

```bash
git add mix.exs mix.lock .gitignore
git commit -m "feat: add Burrito packaging for macos_arm64 + linux_amd64"
```
...
### Task 21: Update justfile + README with build-release recipe
...
```just
# Build single-file binaries for both targets
build-release:
  MIX_ENV=prod mix release

# Build only macOS arm64
build-mac:
  MIX_ENV=prod mix release --target macos_arm64
...
Boxland ships as a single self-contained binary per target via
[Burrito](https://github.com/burrito-elixir/burrito).

### Build locally

```bash
just build-release            # builds both targets to burrito_out/
just build-mac                # macOS arm64 only
just build-linux              # Linux amd64 only
```

First-time build downloads Zig (~70MB) + per-target ERTS (~50MB each).
Subsequent builds are ~30s.

### Install on macOS

```bash
# Download burrito_out/boxland_macos_arm64 from a release
chmod +x boxland_macos_arm64

# First launch hits Gatekeeper (binary isn't notarized in v1):
xattr -d com.apple.quarantine ./boxland_macos_arm64
...
```bash
chmod +x boxland_linux_amd64
./boxland_linux_amd64
```

The TUI opens. Select "Install" on first launch — it will detect/install
dependencies (libvips, Docker), generate config + secrets in `~/.boxland/`,
bring up Postgres + Redis + MinIO via docker-compose, and run migrations.
...
git commit -m "docs: add build-release recipes + binary install instructions"
```
